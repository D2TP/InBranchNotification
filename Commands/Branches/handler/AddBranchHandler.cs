...
namespace InBranchDashboard.Commands.Branches.handler
{
    public class AddBranchHandler : ICommandHandler<BranchCommand>
    {
        ...
        public async Task HandleAsync(BranchCommand command)
        {
            ...
            object[] param1 = { command.region_id };
            var checkRegion = await _dbController.SQLFetchAsync(Sql.SelectOneRegion, param1);
            if (checkRegion.Rows.Count==0)
            {
                _logger.LogError(" returned no result |Caller:BranchController/CreateBranch|| [AddBranchHandler][Handle]");
                throw new HandleGeneralException(400, "Creation failed there is no region with id " + command.region_id);
            }
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(" Server returned no result |Caller:BranchController/CreateBranch|| [AddBranchHandler][Handle]", ex);
