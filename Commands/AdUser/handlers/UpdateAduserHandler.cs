...
namespace InBranchMgt.Commands.AdUser.Handlers
{
    public class UpdateAduserHandler : ICommandHandler<UpdateAduser>
    {
        ...
        public async Task HandleAsync(UpdateAduser command)
        {
            ...
            {
                _logger.LogError("Error: There is no Role with the supplied roleid {roleid}|Caller:ADUserController/UpdateADUser|| [UpdateADUserHandler][Handle]", command.RoleId);
            ...
            role = _convertDataTableToObject.ConvertDataTable<Role>(roleResult).FirstOrDefault();
                    //    email=#,branch_id=#
            object[] paramADUser = {  command.UserName, command.FirstName, command.LastName, command.Active = command.Active,command.Email,command.BranchId, command.id };

            var adUser = _dbController.SQLExecuteAsync(Sql.UpdateADUser, paramADUser) ?? null;

            if (adUser.Result == 0)
            {
                _logger.LogError("Server Error occured, user was not created ||Caller:ADUserController/UpdateADUser  || [UpdateADUserHandler][Handle]", command.UserName);
                throw new HandleGeneralException(500, "Server Error occured");

            }
            object[] paramRole = {  command.id, command.RoleId };
            ...
            {
                _logger.LogError("Server Error occured role was not created||Caller:ADUserController/UpdateADUser  || [UpdateADUserHandler][Handle]", command.UserName);
                throw new HandleGeneralException(500, "Server Error occured");
