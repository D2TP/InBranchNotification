...
namespace InBranchMgt.Commands.CreatUserRole.Handlers
{
    public class CreateUserRoleHandler : ICommandHandler<CreateUserRoleComand>
    {
        ...
        public CreateUserRoleHandler(IMemoryCache memoryCache, IDbController dbController, ILogger<CreateUserRoleHandler> logger, IConvertDataTableToObject convertDataTableToObject, ITracer tracer, IMessageOutbox outbox)
        {
        ...
        {
            //G. OMONI check if param is valid
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("ex syetem error stack: {ex}Error: Server returned no result |Caller:ADUserController/GetAllADUsers-Get|| [GetAllADUserHandler][Handle]",ex);
                throw new HandleGeneralException(500, "Server returned no result");
            }
            //G. OMONI why are we doing spanContext
            var spanContext = _tracer.ActiveSpan.Context.ToString();
            // //G. OMONI my value is the id returned by ado.net
            var @event = new ADUserCreated(entity);
            //G. OMONI why the below
            if (_outbox.Enabled)
            {
